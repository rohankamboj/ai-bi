...
import { FaPlus, FaEdit, FaTimes, FaThumbtack } from "react-icons/fa";
...
const Dashboard: React.FC = () => {
  ...
  const agents = useSelector((state: RootState) => state.user.user?.agents || []);
  const datasources = useSelector((state: RootState) => state.dataSource.dataSources);
  ...
  const [selectedLatitudeField, setSelectedLatitudeField] = useState<string>("");
  const [selectedLongitudeField, setSelectedLongitudeField] = useState<string>("");
  ...
  const handleAddModuleConfirm = () => {
    if (!selectedDatasource || selectedValues.length === 0 || !selectedChartType) {
    ...
    switch (selectedChartType) {
      ...
      case "heatmap":
        if (selectedValues.length !== 3) {
          alert("Please select exactly three values for heatmap: x, y, and value");
          return;
        }
        chartData = flattenedData.map(item => ({
      ...
      case "pie":
        if (selectedValues.length !== 2) {
          alert("Please select exactly two values for pie chart: name and value");
          return;
        }
        chartData = flattenedData.map(item => ({
    ...
    const newComponent: DynamicComponent = {
      ...
      title: `${selectedChartType.charAt(0).toUpperCase() + selectedChartType.slice(1)} Chart - ${selectedSource.name}`,
      data: chartData,
      dataKeys,
      xAxisDataKey,
      nameKey,
      ...
      <div className="h-full">
        {(() => {
          switch (component.type) {
            case "line":
              return <AdvancedLineChart data={component.data} dataKeys={component.dataKeys!} xAxisDataKey={component.xAxisDataKey!} />;
            case "bar":
              return <AdvancedBarChart data={component.data} dataKeys={component.dataKeys!} xAxisDataKey={component.xAxisDataKey!} />;
            case "heatmap":
              return <HeatMapChart data={component.data} xLabel={component.dataKeys![0]} yLabel={component.dataKeys![1]} valueLabel={component.dataKeys![2]} />;
            case "radar":
              return <AdvancedRadarChart data={component.data} dataKeys={component.dataKeys!} nameKey={component.xAxisDataKey!} />;
            case "map":
              return <AdvancedInteractiveMap 
      ...
      <div key={component.id} className="w-full h-full">
        <div className="relative bg-gray-800 rounded-lg shadow-lg p-4 flex flex-col h-full">
          <div className="flex justify-between items-center mb-2 drag-handle cursor-move">
            <h2 className="text-lg font-semibold text-blue-400">{component.title}</h2>
            <div className="flex space-x-2">
              ...
              <button
                ...
                className="no-drag text-gray-300 hover:text-blue-500"
                title={dashboardConfig.lockedComponents.includes(component.id) ? "Unpin Module" : "Pin Module"}
              >
                <FaThumbtack className={`${dashboardConfig.lockedComponents.includes(component.id) ? "text-blue-500" : "text-gray-300"} transition-colors`} />
              </button>
              <button
          ...
          <div className="flex-grow text-gray-300 h-full">
            {editingChart === component.id ? renderEditForm(component.id) : renderChart(component)}
          </div>
        </div>
      </div>
    ));
  ...
  const handleDeleteModule = (id: string) => {
    dispatch(updateDashboard({
      ...dashboardConfig,
      dynamicComponents: dashboardConfig.dynamicComponents.filter(c => c.id !== id),
      layouts: {
        ...dashboardConfig.layouts,
  ...
  const togglePin = (id: string) => {
    ...
    const updatedLockedComponents = isCurrentlyLocked
      ? dashboardConfig.lockedComponents.filter(componentId => componentId !== id)
    ...
    <div className="flex-grow h-screen p-4 overflow-x-hidden bg-gray-900">
      ...
      {isAddModuleModalOpen && (
        ...
        >
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-300">Select Datasource</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300">Select Values</label>
              <select
                multiple
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300">Select Chart Type</label>
              <select
            ...
            {selectedChartType === "map" && (
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-300">Select Latitude Field</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300">Select Longitude Field</label>
                  <select
      ...
      {isAssignAgentModalOpen && (
        ...
        >
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-300">Select Agent</label>
              <select
